...
# ------------------------------------------------------------------- gap --
# Surface-code arithmetic (Fowler et al. 2012 scaling form):
#   p_L(d) ~ A * (p / p_th)^((d+1)/2),  A = 0.1,  p_th = 1e-2.
# Target logical error P_L = 1e-12 per logical operation (RSA-2048 scale).
# Physical qubits per logical qubit ~ 2 d^2 (data + syndrome), no
# distillation factories counted — i.e. deliberately OPTIMISTIC.

P_TH, A_COEF, P_TARGET = 1e-2, 0.1, 1e-12


def d_needed(p):
    """Smallest odd code distance reaching P_TARGET at physical error p."""
    import math
    if p >= P_TH:
        return None
    half = math.log10(A_COEF / P_TARGET) / math.log10(P_TH / p)
    d = math.ceil(2 * half - 1)
    return d + 1 if d % 2 == 0 else d


def phys_per_logical(p):
    d = d_needed(p)
...
def fig_gap():
    import numpy as np
    ps = np.logspace(-4.3, np.log10(8.2e-3), 200)
    n1 = np.array([phys_per_logical(p) for p in ps], dtype=float)
    for mode in ("light", "dark"):
        ink = INK[mode]
        blue = SERIES[mode][0]
        fig, ax = new_fig(mode, (7.0, 4.4))
        ...
        ax.set_yscale("log")
        ax.set_xlim(20, 2e7)
        ax.set_ylim(6e-5, 2.2e-2)
        ax.axhspan(P_TH, 2.2e-2, color=ink["grid"], alpha=0.55, zorder=0)
        ax.text(1.2e7, 1.35e-2, "above threshold — no code can help",
                ha="right", color=ink["muted"], fontsize=8, style="italic")
        # frontiers
        ax.fill_betweenx(ps, n1, 2e7, color=ink["grid"], alpha=0.35,
                         zorder=0)
        ax.plot(n1, ps, color=ink["secondary"], lw=1.4, linestyle="--",
                zorder=2)
        ax.plot(n1 * 1000, ps, color=ink["secondary"], lw=1.4, zorder=2)
        ax.text(2.6e3, 8.5e-5, "1 logical qubit\n(@ 10⁻¹² error)",
                color=ink["secondary"], fontsize=8, ha="left")
        ax.text(2.6e6, 8.5e-5, "1,000 logical\nqubits",
                color=ink["secondary"], fontsize=8, ha="left")
        # destinations
        for x, y, lab, dx, ha in [
                (1e6, 1e-3, "RSA-2048\n(Gidney 2025)", -10, "right"),
                (4e6, 1e-3, "FeMoco chemistry\n(Lee et al. 2021)", 10, "left")]:
            ax.scatter(x, y, s=130, marker="*", color=ink["primary"],
                       zorder=4, edgecolors=SURFACE[mode], linewidths=0.8)
            ax.annotate(lab, (x, y), xytext=(dx, 8),
                        textcoords="offset points", ha=ha,
                        color=ink["primary"], fontsize=8)
        # today's machines
        pts = scatter_points()
        for p in pts:
            face = SURFACE[mode] if p["spec"] else blue
            ax.scatter(p["q"], p["e"], s=44, facecolors=face,
                       edgecolors=blue, linewidths=1.4, zorder=3)
        ax.annotate("every quantum computer on Earth, 2026",
                    xy=(150, 2.4e-3), xytext=(28, 2.3e-4),
                    color=ink["secondary"], fontsize=8.5,
                    arrowprops=dict(arrowstyle="->", color=ink["secondary"],
                                    lw=0.9))
        ax.set_xticks([1e2, 1e3, 1e4, 1e5, 1e6, 1e7],
                      ["100", "1k", "10k", "100k", "1M", "10M"])
        ax.set_yticks([1e-4, 1e-3, 1e-2], ["0.01%", "0.1%", "1%"])
        ax.xaxis.set_minor_formatter(NullFormatter())
        ax.yaxis.set_minor_formatter(NullFormatter())
        ax.set_xlabel("physical qubits (log)")
        ax.set_ylabel("two-qubit gate error (log)")
        ax.set_title("The gap, computed: where useful fault tolerance "
                     "lives vs where the machines are")
        save(fig, "fig-gap", mode)


def emit_gap_table():
    """Write machines/_gap.md — surface-code arithmetic per machine."""
    lines = [
    ...
    for p in sorted(scatter_points(), key=lambda x: x["e"]):
        d = d_needed(p["e"])
        per = 2 * d * d
        nlog = p["q"] // per
        name = p["name"] + (" *(spec)*" if p["spec"] else "")
        lines.append(f"| {name} | {p['e']:.1e} | {d} | {per:,} | **{nlog}** |")
    lines += [
        "| *needed for RSA-2048* | *~10⁻³* | *21* | *~900* | "
        "***~1,400 (+ factories)*** |",
        "",
        "*Model: surface code, $p_L = 0.1\\,(p/p_{th})^{(d+1)/2}$, "
        "$p_{th}=1\\%$, target $10^{-12}$ per logical operation, $2d^2$ "
        "physical qubits per logical — **no distillation factories counted, "
        "i.e. optimistic**. At $p=10^{-3}$ this simple model prices 1,000 "
        "logical qubits at ≈880k physical — within 12% of Gidney's detailed "
        "RSA-2048 estimate, independently. The arithmetic is robust.*",
    ]
    out = ROOT / OUT / "_gap.md"
    out.write_text("\n".join(lines) + "\n")
    print(f"  {out.relative_to(ROOT)}")


CAP_COLS = [("mid_circuit_meas", "mid-circuit meas."),
            ("feed_forward", "feed-forward"),
            ("qubit_reuse", "qubit reuse"),
            ("arb_angle_2q", "arb-angle 2Q"),
            ("pulse_access", "pulse access"),
            ("open_access", "open access")]


def emit_caps_table():
    """Write machines/_caps.md — the capability matrix for gate-based machines."""
    sym = {True: "✓", False: "✗", None: "—"}
    order = {m["id"]: (SLOT[m["modality"]], -(m["qubits"] or 0))
             for m in DATA["machines"]}
    rows = sorted((m for m in DATA["machines"] if "caps" in m),
                  key=lambda m: order[m["id"]])
    lines = [
        "<!-- GENERATED by scripts/make_machine_figures.py from data.yml — do not edit. -->",
        "",
        "| Machine | " + " | ".join(lbl for _, lbl in CAP_COLS) + " | The fine print |",
        "|---|" + ":-:|" * len(CAP_COLS) + "---|",
    ]
    for m in rows:
        cells = " | ".join(sym[m["caps"][k]] for k, _ in CAP_COLS)
        lines.append(f"| [{m['name']}]({m['page']}) | {cells} | "
                     f"{m.get('caps_note', '')} |")
    lines += [
        "",
        "✓ available · ✗ not available/not exposed · — unknown or "
        "undocumented. Analog machines (Aquila, Pasqal, Jiuzhang, D-Wave) "
        "are omitted: these columns are circuit-model concepts.",
    ]
    out = ROOT / OUT / "_caps.md"
    out.write_text("\n".join(lines) + "\n")
    print(f"  {out.relative_to(ROOT)}")


def emit_claims_table():
    """Write machines/_claims.md — the advantage-claim ledger."""
    badge = {"matched": "**MATCHED**", "reduced": "**REDUCED**",
             "standing": "standing"}
    lines = [
        "<!-- GENERATED by scripts/make_machine_figures.py from data.yml — do not edit. -->",
        "",
        "| Date | Machine | Advertised | Status | Where it stands (as of "
        f"{DATA['as_of']}) |",
        "|---|---|---|---|---|",
    ]
    tally = {"matched": 0, "reduced": 0, "standing": 0}
    for c in sorted(DATA["claims"], key=lambda c: c["date"]):
        tally[c["status"]] += 1
        cell = f"[{c['machine']}]({c['page']})"
        cur = " ".join(c["current"].split())
        if c["attacks"]:
            cur += " *(" + "; ".join(c["attacks"]) + ")*"
        lines.append(f"| {c['date']} | {cell} | {c['advertised']} | "
                     f"{badge[c['status']]} | {cur} |")
    lines += [
        "",
        f"**Score: {tally['matched']} matched, {tally['reduced']} reduced, "
        f"{tally['standing']} standing** — of the "
        f"{sum(tally.values())} headline advantage claims since 2019, "
        f"{tally['matched'] + tally['reduced']} have been killed or cut "
        "down by classical algorithms. Every *standing* claim is either "
        "less than ~2 years old or deliberately modest.",
    ]
    out = ROOT / OUT / "_claims.md"
    out.write_text("\n".join(lines) + "\n")
    print(f"  {out.relative_to(ROOT)}")
